...
<div class="col-md-12">
    <div class="row">
        ...
        <!-- Sidebar (Post Titles + Topic Name) -->
        <div class="col-md-3">
            <div class="sticky-top" style="top: 70px;">
                <div class="bg-light rounded shadow-sm p-3 mb-3">
                    <h6 class="text-primary fw-bold mb-3">
                        <?= esc($topic['name']) ?> <!-- Changed from project -->
                    </h6>
                    <ul class="list-group" id="postList">
                        <?php foreach ($posts as $index => $post): ?>
                            <li class="list-group-item post-title-1 <?= $index === 0 ? 'active-title' : '' ?>" 
                                data-id="<?= $post['id'] ?>">
                                <?= esc($post['title']) ?>
                            </li>
                        <?php endforeach; ?>
        ...
        <div class="col-md-9">
            ...
            <?php foreach ($posts as $index => $post): ?>
                <div class="post-content <?= $index === 0 ? '' : 'd-none' ?>" id="post-<?= $post['id'] ?>">
                    ...
                    <div><?= $post['content'] ?></div> <!-- Removed nl2br for HTML formatting -->
                </div>
            <?php endforeach; ?>
        </div>
    </div>
